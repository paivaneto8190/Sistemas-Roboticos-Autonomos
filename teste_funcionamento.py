...
def distMin (ponto_atual, ref, camTam) :
    deltaL = 50000
    d = 0
    i = 0
    x = 0
...
def segueTrajeto (q,v_q, q_ref,vq_ref):
...
errorCode, refPoint = sim.simxGetObjectHandle(clientID, 'ref_point', sim.simx_opmode_oneshot_wait)
...
qi = np.array([0, 0, 70])
qf = np.array([-1.5,1.875, 80])
step = 0.01

X, Y, teta, a, b = Poli3(qi, qf, step)
ks = curve(X, Y)
...
errorCode, velocityRobot = sim.simxGetObjectVelocity(clientID, robot, sim.simx_opmode_streaming)

for i in range(100):
    #Captura dados da configuração dos robôs
    erro, posRobot = sim.simxGetObjectPosition(clientID, robot, -1, sim.simx_opmode_buffer)
    error, velRobor = sim.simxGetObjectVelocity(clientID, robot, sim.simx_opmode_buffer)
    erro, [alpha, beta, gamma] = sim.simxGetObjectOrientation(clientID, robot, -1, sim.simx_opmode_buffer)
    erro, posRef = sim.simxGetObjectPosition(clientID, refPoint , -1, sim.simx_opmode_buffer)
    error, velRef = sim.simxGetObjectVelocity(clientID, refPoint, -1, sim.simx_opmode_buffer) #colocar o refPoint
    x_pos.append(posRobot[0])
    y_pos.append(posRobot[1])
    teta.append((gamma*180)/mt.pi)

    posiAtual = Pose_atual(robot) #Coordenadas atuais do robo
    v_posicao = sim.simxGetObjectVelocity(clientID, robot, -1, sim.simx_opmode_buffer)
    distMin_res = distMin(posiAtual, posRef), len(X)
    deltaTeta_res = deltaTeta(posiAtual, distMin_res)

    W = samson(v, ks, deltaTeta_res, distMin_res[2])

    wd, we = segueCaminho(posiAtual, W)

    #Imprime os dados da configuração do robô no CoppeliaSim
    sim.simxAddStatusbarMessage(clientID, "Posição em x " + str(posRobot[0]) + " e a Posição em y " +str(posRobot[1]), sim.simx_opmode_oneshot_wait)
    ...
    #Motores
    errorCode = sim.simxSetJointTargetVelocity(clientID, left_motor_handle, we, sim.simx_opmode_oneshot_wait)
    errorCode = sim.simxSetJointTargetVelocity(clientID, right_motor_handle, wd, sim.simx_opmode_oneshot_wait)
